...
export default function SubmitPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("http://localhost:4242/audit", {
        method: "POST",
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#0b0d12", color: "#e6e8ee" }}>
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "52px 48px 72px" }}>
        ...
        {status === "submitted" ? (
          <div style={{ marginTop: 14, borderLeft: "3px solid #34d399", backgroundColor: "#11141b", border: "1px solid #1f2330", borderRadius: 10, padding: 18 }}>
            <div style={{ fontWeight: 700, color: "#34d399", marginBottom: 4 }}>Submission received</div>
            <div style={{ color: "#a9b1c1", fontSize: 13.5 }}>We are cloning and auditing {repoUrl || "your repo"} now. The report will be emailed to {email}.</div>
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={{ backgroundColor: "#11141b", border: "1px solid #1f2330", borderRadius: 10, padding: 18 }}>
        ...
        <div style={{ marginTop: 40, color: "#7d879a", fontSize: 11, borderTop: "1px solid #273247", paddingTop: 10, textAlign: "right" }}>
